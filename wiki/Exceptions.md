...
| `ColumnError` | `DatasetError` | Column is missing, duplicated, or has the wrong dtype. Message lists every missing name plus the full available set. | `check_columns_exist`, `validate_dataframe_column`, `to_datetime_index` (missing index column), `Query.where`, `Query.column_has_missing` / `column_is_empty`, `Query.where_values_between`, `MultiGasData.add_wind_direction`, `MultiGasData.add_wind_quadrant`, `Query.select_columns` / `select_numeric_columns` (when `validate=True`). |
...
| `LoaderError` | `MultigasException` | Source file cannot be located, parsed, or normalised, or `dataset_type` / `index_col` is invalid. | `DataLoader.load`, `DataLoader._load_csv`, `read_file`. Any exception raised while reading, normalising, running the sampling-consistency check, or writing the cache is re-raised as `LoaderError` (chained via `from e`) — so a `CacheError` from `save_cache` surfaces to callers of `load` as a `LoaderError`. |
| `MetadataError` | `MultigasException` | TOA5 (or equivalent) metadata cannot be extracted. | Reserved for future TOA5 metadata parser. |
| `PlotError` | `MultigasException` | A plotting routine cannot render its output. | Currently unused. `plot_completeness` treats a plotting failure as a **soft failure** — it logs a `WARNING` and returns `None` instead of raising, so `extract_daily` never aborts on a bad plot. Reserved for future plotting routines where a failed render is a real error. |
